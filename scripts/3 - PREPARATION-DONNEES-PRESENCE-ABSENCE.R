...
resultats_bruts_lots_plusieurs_crottes %>%
  dplyr::select(id_lot, Conclusion) %>%
  dplyr::rename(espece = Conclusion) -> plusieurs_crottes
...
# importation des shapes des points de collecte 
points_collecte_lots_2020<-sf::st_read(dsn = "data/crotte", layer = "points_collecte_lots_2020")
points_collecte_lots_2020 %>% 
  sf::st_cast("POINT") -> points_collecte_lots_2020 # ici cette fonction car la géométrie "multipoint" n'est pas acceptée lors de la rasterisation
...
points_collecte_lots_2020 %>%
  dplyr::left_join(crottes_assemblees, by = c("id" = "id_lot")) %>%
  dplyr::filter(!is.na(timidus)) -> points_collecte_espece 


# rasterisation des points timidus et europaeus
raster<-raster::raster("./data/rasters/alt_PNM_focal400.asc")
projection(raster)<-CRS("+init=EPSG:2154")
raster[]<-0
raster_timidus<-raster::rasterize(points_collecte_espece, field="timidus", raster,  background=NA)
raster_europaeus<-raster::rasterize(points_collecte_espece, field="europaeus", raster,  background=NA)
...
polygon_timidus<-raster::rasterToPolygons(raster_timidus)
...
polygon_europaeus<-raster::rasterToPolygons(raster_europaeus)
...
raster::projection(long_traces)<-CRS("+init=EPSG:2154")
traces <- long_traces
traces[traces == 0] <- NA
...
# création dataframe avec les coordonnées (centroïdes) des mailles 100m prospectées et une colonne "pres" avec 0.
polygon_traces<-raster::rasterToPolygons(traces)
coord_traces<-as.data.frame(raster::coordinates(polygon_traces))
pres <- vector(length = dim(coord_traces)[1])
pres[]<-0
coord_traces <- cbind(coord_traces, pres)
colnames(coord_traces)<-c("x","y","pres")

# le tableau de coordonnées avec présences (voir au-dessus)
...
# assemblage des deux tableaux et suppression des doublons (centroïdes avec lièvre et traces) en ne conservant  que les 1 
